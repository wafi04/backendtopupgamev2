...
export class VerificationTokenRepository {
  private  handlePrismaError(error: unknown): never {
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      switch (error.code) {
        case 'P2002':
          throw new ApiError(409, ERROR_CODES.CONFLICT, "Verification token already exists");
        case 'P2025':
   ...
   async verifyToken(token: string, otp?: string) {
    try {
      ...
      if (!verificationToken) {
        throw new ApiError(404, ERROR_CODES.NOT_FOUND, "Invalid verification token");
      }

      if (verificationToken.expires < new Date()) {
        throw new ApiError(410, ERROR_CODES.SESSION_EXPIRED, "Verification token has expired");
      }
