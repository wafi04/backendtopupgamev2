...
import { CreateMethod,  FilterMethod, UpdateMethods } from "@/common/interfaces/methods";
...
MethodsRoutes.patch('/:id',ContextAwareMiddleware.roleMiddleware([ADMIN_ROLE]), asyncHandler(async(req : Request,res : Response) => {
...
MethodsRoutes.delete('/:id',ContextAwareMiddleware.roleMiddleware([ADMIN_ROLE]), asyncHandler(async(req : Request,res : Response) => {
    const id   = req.params.id
...
MethodsRoutes.get('/', asyncHandler(async(req : Request,res : Response) => {
